...
var gnb = function(){
	...
	var open = function(){
		$dim.stop().fadeIn(200);
		$pane.stop().animate({ left: 0 },500,"easeOutQuart");
	}

	var close = function(){
		$dim.stop().fadeOut(100);
		$pane.stop().animate({ left: "-80%" },300,"easeOutQuart");
	}
		
	$el.on("click", ".top-wrap > .h-btn",".gnb ul li a", function(e){
		if($el.is(".mp-header")){
	...
	$el.on("click", ".gnb > ul > li > a", function(e){
		$el.find(".top-wrap > .h-btn").removeClass("active");
		$("body").css({overflow : "visible"});
		close();
	});
...
var newsAttach = function(){
	var attach = $('.news-view .body .attach');
	if(attach.find('.file').length < 1){
		attach.hide();
	}

}

/* Visual scroll button */
var newsAttach = function(){
	var $btn = $('.slide-btn');
	$btn.on('click', function(e){
		e.preventDefault();
		...
		$('html, body').animate({
			scrollTop: $(linkScroll).offset().top - $('.main-header').outerHeight() + 10
		}, 200);
